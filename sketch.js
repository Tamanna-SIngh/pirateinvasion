...
var isLaughing= false; //flag is a generic term for a variable that holds just two value : true or false
...
function draw() {
  ...
  if (!backgroundMusic.isPlaying()) {
    ...
    // values between 0(mute) and 1(highest)
  }
...
function showBoats() {
  if (boats.length > 0) {
    ...
    // Boolean NOT operator (!) = checks opposite thing
      ...
      if (collision.collided && !boats[i].isBroken) {
          ...
          if(!isLaughing && !pirateLaughSound.isPlaying()){
            ...
            //pirateLaugh.isPlaying() = !(false) = true (LHS)
            // RHS = true
